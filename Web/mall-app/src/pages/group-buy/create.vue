<template>
  <view class="create-page">
      ...
      <view class="custom-card">
        ...
        <view class="options-wrap">
          ...
          <view class="option-panel">
            ...
            <!-- 批量折扣提示 -->
            <view class="discount-tip" v-if="batchDiscount">
              <uni-icons type="info" size="16" color="#ff6b6b" />
              <text>{{ batchDiscount }}</text>
            </view>
          </view>
    ...
    <view class="bottom-bar">
      ...
      <button class="submit-btn" @click="handleCreate" :loading="submitting">发起拼团</button>
    </view>
...
import { getActivityDetailApi, createGroupBuyApi, calculateBatchPriceApi } from '@/api/groupBuy'
...
const calculatePrice = async () => {
  try {
    ...
    batchDiscount.value = res?.discountRule || ''
  } catch (e) {
...
const handleCreate = async () => {
  ...
  try {
    ...
    const res = await createGroupBuyApi(requestData)

    uni.showToast({ title: '发起成功', icon: 'success' })

    setTimeout(() => {
      uni.redirectTo({
        url: `/pages/group-buy/group-detail?id=${res?.groupId}`
      })
    }, 1000)
  } catch (e) {
